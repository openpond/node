...
export class P2PNetwork extends EventEmitter {
    ...
    updateInterval = null;
    lastDHTUpdate = 0;
    MIN_DHT_UPDATE_INTERVAL = 10_000; // 10 seconds instead of 60 seconds
    MAX_CONNECTIONS = this.bootstrapMode ? 1000 : 50;
    bootstrapNodes = [];
    nodeStatuses = new Map();
    knownPeerToEthMap = new Map();
    knownAgentNames = new Map();
    // Constants for DHT operations
    DHT_OPERATION_TIMEOUT = 30000; // 30 seconds
    DHT_GET_TIMEOUT = 10000; // 10 seconds
    DHT_PUT_TIMEOUT = 20000; // 20 seconds
    /**
    ...
    async verifyBootstrapNode() {
        if (this.bootstrapMode) {
            try {
                const keyData = JSON.parse(await fs.readFile(`./bootstrap-keys/${this.agentName}-peer.json`, "utf8"));
