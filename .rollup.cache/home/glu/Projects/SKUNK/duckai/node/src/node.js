...
async function initNode() {
    ...
    p2p = new P2PNetwork(config.privateKey, config.name, config.version, config.metadata, config.registryAddress, config.rpcUrl, config.network, config.useEncryption);
    ...
    if (config.nodeType === "bootstrap") {
        Logger.info("Node", "Starting bootstrap node", {
            name: config.name,
            expectedPeerId: BOOTSTRAP_PEER_IDS[config.name],
        });
