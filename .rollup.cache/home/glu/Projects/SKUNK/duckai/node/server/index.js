...
async function main() {
    ...
    const server = new APIServer(parseInt(process.env.PORT || process.env.API_PORT || "3000"), process.env.API_PRIVATE_KEY, process.env.REGISTRY_ADDRESS, process.env.RPC_URL, (process.env.NETWORK || "base"));
    try {
        await server.start();
