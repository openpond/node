...
export class TestRunner extends EventEmitter {
    ...
    results = new Map();
    funderClient;
    constructor(registryAddress, rpcUrl, funderPrivateKey // Private key of the wallet with funds
    ) {
        super();
    ...
    async startBootstrapNodes(count = 2) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (balance === 0n) {
                ...
                Logger.info("Test", `Funded bootstrap node ${i} at address ${account.address}`);
            }
            // Create bootstrap node
            const node = new P2PNetwork(privateKey, `bootstrap-${i}`, "1.0.0", {}, // Bootstrap nodes don't need capabilities
            this.registryAddress, this.rpcUrl);
            await node.start(port);
            await node.registerWithContract(); // Register bootstrap nodes too
            this.bootstrapNodes.push(node);
            Logger.info("Test", `Bootstrap node ${i} started on port ${port} with address ${node.getAddress()}`);
        }
    ...
    async createAgents(count) {
        ...
        const bootstrapAddresses = this.bootstrapNodes.map((node) => node.getAddress());
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (balance === 0n) {
                const funded = await this.fundAgent(account.address);
                if (!funded) {
                    throw new Error(`Failed to fund agent ${i} at address ${account.address}`);
                }
            ...
            const node = new P2PNetwork(privateKey, `test-agent-${i}`, "1.0.0", {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                try {
                    ...
                    if (records[address]) {
                        published = true;
                        Logger.info("Test", `Agent ${i} DHT record published successfully`, {
                    ...
                    Logger.warn("Test", `Attempt ${attempt + 1} to verify DHT record failed`, { error });
                }
                await new Promise((resolve) => setTimeout(resolve, 5000));
            }
            if (!published) {
    ...
    async runMessageTest() {
        ...
        try {
            ...
            if (result.messagesReceived !== expectedMessages) {
                throw new Error(`Expected ${expectedMessages} messages, but received ${result.messagesReceived}`);
            }
        }
        catch (error) {
