...
async function startChatClient() {
    ...
    if (!process.env.PRIVATE_KEY ||
        ...
        !process.env.RPC_URL) {
        throw new Error("Missing required configuration. Need: name, privateKey, registryAddress, rpcUrl");
    }
    Logger.info("ChatClient", "Initializing agent", {
        agent: config.name,
        port: config.port,
    });
    const node = new P2PNetwork(config.privateKey, config.name, "1.0.0", {}, config.registryAddress, config.rpcUrl, config.network, false);
    try {
        ...
        if (input === "/help" || input === "/h") {
            ...
            try {
                if (input.startsWith("@")) {
                    ...
                    console.log(chalk.red('Invalid format. Use "@address message" or "all: message"'));
                }
            }
            catch (error) {
