...
async function cleanLogFile(agentNum) {
    const agentName = process.env.AGENT_NAME || `agent-${agentNum}`;
...
async function getAIResponse(message, fromAgent) {
    try {
        ...
        const messages = [
            {
                role: "system",
                content: process.env.AGENT_NAME === "Ducky"
...
export async function startExplorer() {
    ...
    function highlightInstruction(key) {
        const instructions = [
            "🎮 Controls:",
            ` [{${key === "Tab" ? agentColor : "white"}-fg}Tab{/}] - Switch between panels`,
            ` [{${key === "i" ? agentColor : "white"}-fg}i{/}] - Focus input box`,
            ` [{${key === "p" ? agentColor : "white"}-fg}p{/}] - Focus peer list`,
            ` [{${key === "Enter" ? agentColor : "white"}-fg}Enter{/}] - Select peer/Send message`,
    ...
    try {
        ...
        const network = new P2PNetwork(process.env.PRIVATE_KEY || "", process.env.AGENT_NAME || `agent-${agentNum}`, "1.0.0", {}, process.env.REGISTRY_ADDRESS, process.env.RPC_URL);
        // Log when network is started
        await network.start(parseInt(process.env.EXPLORER_PORT || defaultPort.toString()));
        ...
        network
            .getLibp2p()
            .services.pubsub.addEventListener("message", async (evt) => {
            if (evt.detail.topic === "agent-messages") {
                try {
                    ...
                    if (messageWrapper && messageWrapper.message) {
                        ...
                        if (msg.toAgentId.toLowerCase() ===
                            network.getAddress().toLowerCase()) {
                            Logger.info("Explorer", "Message is for me, waiting for decryption");
                        }
                    }
                    screen.render();
                }
        ...
        network.getLibp2p().addEventListener("peer:connect", async (evt) => {
        ...
        network.getLibp2p().addEventListener("peer:disconnect", (evt) => {
            const peerId = evt.detail.toString();
        ...
        network.on("message", async (message) => {
            logBox.log(`[DEBUG] Got decrypted message event: ${JSON.stringify(message)}`);
            if (message.toAgentId.toLowerCase() === network.getAddress().toLowerCase()) {
                logBox.log(`[DEBUG] Displaying decrypted message in chat`);
                chatBox.log(`{${agentColor}-fg}${message.fromAgentId.slice(0, 10)}...{/}: ${message.content}`);
                // Generate and send AI response
                try {
                    const aiResponse = await getAIResponse(message.content, message.fromAgentId);
                    ...
                    // Display in our chat
                    chatBox.log(`{${agentColor}-fg}You{/} to ${message.fromAgentId.slice(0, 10)}...: ${aiResponse}`);
                }
                catch (error) {
                    Logger.error("Explorer", "Failed to send AI response", { error });
                    chatBox.log(`Error sending AI response: ${error instanceof Error ? error.message : String(error)}`);
                }
                screen.render();
            }
        });
        ...
        messageEvents.forEach((event) => {
            network.on(event, (message) => {
                logBox.log(`[DEBUG] ${event} event: ${JSON.stringify(message)}`);
                screen.render();
            });
        });
        // Handle peer selection once, outside updateNetworkInfo
        peerSelection.on("select", async (item) => {
            const agentName = item.content;
            const dhtRecords = (await network.getDHTRecords());
            const record = Object.entries(dhtRecords).find(([_, r]) => r.agentName === agentName);
            if (record) {
        ...
        inputBox.key(["enter"], async () => {
            ...
            if (message.trim()) {
                if (!selectedPeer) {
                    chatBox.log("Please select a peer first (use Tab to focus peer list)");
                }
                else {
                    try {
                        ...
                        chatBox.log(`{${agentColor}-fg}You{/} to ${selectedPeer.ethAddr.slice(0, 10)}...: ${message}`);
                        ...
                        chatBox.log(`Error sending message: ${error instanceof Error ? error.message : String(error)}`);
                        screen.render();
                    }
                }
            }
        ...
        const updateNetworkInfo = async () => {
            try {
                ...
                if (!knownAgents.size) {
                    setTimeout(() => {
                        HARDCODED_AGENTS.forEach((agent) => {
                            chatBox.log(`{white-fg}${agent}{/white-fg} has joined the network!`);
                        });
                        screen.render();
                    }, 2000); // 2 second delay
                }
                ...
                const newRealAgents = Object.entries(dhtRecords)
                    .map(([ethAddr, record]) => record.agentName || `Agent ${ethAddr.slice(0, 6)}`)
                ...
                if (selectedPeer) {
                    const isPeerConnected = connectedPeers.some((p) => p.toString() === selectedPeer?.peerId);
                    const selectedRecord = dhtRecords[selectedPeer.ethAddr.toLowerCase()];
                    const selectedAgentName = selectedRecord?.agentName ||
                        `Agent ${selectedPeer.ethAddr.slice(0, 6)}`;
                    content += "🎯 Selected Agent:\n";
                    content += `Name: {white-fg}${selectedAgentName}{/white-fg}\n`;
                    content += `Status: ${isPeerConnected ? "🟢 Connected" : "🔴 Offline"}\n`;
                }
...
//# sourceMappingURL=agent-explorer.js.map